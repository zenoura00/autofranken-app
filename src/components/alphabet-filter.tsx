    ...
    <div className={className}>
      <div className="flex flex-wrap items-center gap-2 mb-6">
        <button
          ...
          className={
            active === "ALL"
              ? "px-3 py-1 rounded-md bg-emerald-600 text-white text-sm"
        ...
        {letters.map(letter => {
          ...
          return (
            <button
              ...
              className={
                isActive
                  ? "px-2.5 py-1 rounded-md bg-emerald-600 text-white text-sm"
                  : isDisabled
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(i => (
            <div key={i.id} className="border rounded-lg p-4 hover:shadow-sm transition">
              {i.href ? (
                <Link className="font-semibold text-lg mb-1 text-emerald-600 hover:text-emerald-700 underline" href={i.href}>
                  {i.label}
                </Link>
              ) : (
              ...
              {i.links && i.links.length > 0 ? (
                <div className="flex flex-wrap gap-3 text-sm">
                  {i.links.map(l => (
                    <Link key={l.href} className="text-emerald-600 hover:text-emerald-700 underline" href={l.href}>
                      {l.label}
                    </Link>
                  ))}
                </div>
